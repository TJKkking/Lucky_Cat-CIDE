...
{   //新建按钮
    ...
    //插入到菜单栏
    QMenu *file1 = menuBar()->addMenu(tr("&文件"));
...
void luckycat::editmenu()
{
    ...
    QMenu *file2 = menuBar()->addMenu(tr("&编辑"));
...
void luckycat::runmenu()
{
    ...
    QMenu *file3 = menuBar()->addMenu(tr("&运行"));
...
void luckycat::splitter()
{
    ...
    QTextEdit *textLeft=new QTextEdit(splitterMain);
    ...
    textMain=new QTextEdit(splitterRight);
    QTextEdit *textBottom =new QTextEdit(splitterRight);
    ...
    splitterRight->setStretchFactor(0,9);
...
void luckycat::on_open()
{
    ...
    {
        textMain->setPlainText(p.readAll());
        p.close();
